...
function analyzeSalesData(data, options) {
    ...
    if (!data 
        ...
        throw new Error('Некорректные входные данные по customers');
    }
    if(
        !Array.isArray(data.products)
        || data.products.length === 0
    ) {
        throw new Error('Некорректные входные данные по products');
    }
    if(
        !Array.isArray(data.sellers)
        || data.sellers.length === 0
    ) {
        throw new Error('Некорректные входные данные по sellers');
    }
    if(
        !Array.isArray(data.purchase_records)
        || data.purchase_records.length === 0
    ) {
        throw new Error('Некорректные входные данные по purchase_records');
    }
    ...
    if (typeof options !== "object") {
        throw new Error('В options передан не объект');
    }

    const { calculateRevenue, calculateBonus } = options;
    if (typeof calculateRevenue !== "function" || typeof calculateBonus !== "function") {
        throw new Error('В options передана не функция');
    }
    ...
    data.purchase_records.forEach(record => { // Чек 
        ...
        record.items.forEach(item => {
            ...
            // Посчитать себестоимость (cost) товара как product.purchase_price, 
